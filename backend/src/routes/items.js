...
router.get('/', optionalAuth, async (req, res) => {
  ...
  if (req.user) {
    ...
    const { rows } = await pool.query(
      `SELECT i.id, i.description, i.list_price, i.low_price, i.sold_price, i.picked_up,
...
router.post('/', authRequired, async (req, res) => {
  const { consignor_id, description, list_price, low_price, sold_price, picked_up } = req.body || {};
  if (!consignor_id || !description) {
    return res.status(400).json({ error: 'consignor_id and description required' });
  }
  const ins = await pool.query(
    `INSERT INTO items (consignor_id, description, list_price, low_price, sold_price, picked_up)
     VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
    [consignor_id, description, list_price ?? null, low_price ?? null, sold_price ?? null, !!picked_up]
  );
  res.json(ins.rows[0]);
});

router.put('/:id', authRequired, async (req, res) => {
  const { description, list_price, low_price, sold_price, picked_up } = req.body || {};
  if (!description) return res.status(400).json({ error: 'description required' });
  const upd = await pool.query(
    `UPDATE items SET description=$1, list_price=$2, low_price=$3, sold_price=$4, picked_up=$5
     WHERE id=$6 RETURNING *`,
    [description, list_price ?? null, low_price ?? null, sold_price ?? null, !!picked_up, req.params.id]
  );
